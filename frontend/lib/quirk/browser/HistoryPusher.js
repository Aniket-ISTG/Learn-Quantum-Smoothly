...
class HistoryPusher {
    ...
    stateChange(stateObj, stateUrlHash) {
        ...
        try {
            const safeState = (typeof stateObj === "object" && stateObj !== null) ? stateObj : { quirkState: stateObj };
            // 'Memorable' states should stay in the history instead of being replaced.
            if (this._currentMemorableStateObj === undefined) {
                history.replaceState(safeState, "", stateUrlHash);
            } else {
                history.pushState(safeState, "", stateUrlHash);
                this._currentMemorableStateObj = undefined;
            }
        } catch (ex) {
