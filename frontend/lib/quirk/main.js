...
export function initQuirk(options = {}) {
    if (typeof window === "undefined") return null;

    const canvasDiv = document.getElementById("canvasDiv");
    if (!canvasDiv) return null;
    const canvas = /** @type {!HTMLCanvasElement} */ (document.getElementById("drawCanvas"));
...
function setupYellowModeToggle() {
    ...
    canvas.height = Math.max(window.innerHeight * 0.7, 650);
...
obsForgeIsShowing.
    ...
    haveLoaded = true;
    setTimeout(() => {
        if (inspectorDiv) inspectorDiv.style.display = 'block';
        redrawNow();
        const closeBtn = document.getElementById("close-circuits-button");
        if (closeBtn) {
    ...
    if (options.onCircuitChange) {
        revision.latestActiveCommit().subscribe(jsonText => {
            try {
                options.onCircuitChange(jsonText);
            } catch (err) {
                console.error(err);
            }
        });
    ...
    if (options.initialCircuit) {
        try {
            let circuitDef = fromJsonText_CircuitDefinition(options.initialCircuit);
            let newInspector = displayed.get().withCircuitDefinition(circuitDef);
            displayed.set(newInspector);
            revision.commit(newInspector.snapshot());
        } catch (err) {
            console.error("Failed to load initial circuit:", err);
        }
    }
    ...
    return {
        ...
        displayed,
        mostRecentStats,
        redraw: () => redrawThrottle && redrawThrottle.trigger(),
        getCircuitJson: () => revision.latestActiveCommit().get(),
        setCircuitJson: (jsonText) => {
            let circuitDef = fromJsonText_CircuitDefinition(jsonText);
            let newInspector = displayed.get().withCircuitDefinition(circuitDef);
            displayed.set(newInspector);
            revision.commit(newInspector.snapshot());
